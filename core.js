...
function crateCoordinates(input) {
    return [input[0]];
}
function drawCircles(coords, layer) {
    var circleRadious = 300; // meters
...
function destinationPoint(coordinate, distance, bearing) {
    ...
    var destinationPoint = L.latLng(toDegrees(φ2), (toDegrees(λ2) + 540) % 360 - 180); // normalise to −180..+180°,
    return {
...
function main() {
    var targetAreaCoords = rawAreaCoords.map(function (single) {
        return L.latLng(single);
    });
    var coords = crateCoordinates(targetAreaCoords);
    var circlesLayer = new L.FeatureGroup().addTo(map);
    L.polygon(targetAreaCoords).addTo(map);
    drawCircles(coords, circlesLayer);
}
var current = { lat: 42.688138888888886, lng: 23.292833333333334 };
